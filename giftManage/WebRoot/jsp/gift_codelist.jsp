...
.tdLabel {
    ...
    <%
      //和ListGift_codeByNameAction.java文件里定义的默认值保持一致即可
      // 定义如下分页变量
    ...
    <%
     ...
     %>
        ...
        <br>
        <s:form name="form1" action="gift_codelistByName.action" method="post" >
            <table border="1" style="BORDER-COLLAPSE: collapse" width="35%"
                ...
                <tr align=center  bgcolor="#CCFF66">
                    <td colspan="2">
                     ...
                     <s:textfield id="name_c" name="name_c" label="name"></s:textfield>
        ...
        <table width="80%" align="center">
         <s:url id="url_savegift_code" action="gift_codesaveP">
             <s:param name="name_c"><s:property value="#request.name_c" /></s:param>
         </s:url>
         <s:a href="%{url_savegift_code}">添加新记录</s:a>
        </table>
        <hr color="#CCCCFF">
        <table border="1" width="80%" align="center" bgcolor="#FFFFCC"
            ...
            <s:iterator value="#request.list" id="gift_code" >
                <tr align="center" onmousemove="return this.bgColor='#FDF525';"  onmouseout="return this.bgColor='';">
                 <td >
                        <s:property value="#gift_code.pk_id" />
                </td>
                 <td >
                        <s:property value="#gift_code.name" />
                </td>
                 <td >
                        <s:property value="#gift_code.code" />
                </td>
                 <td >
                        <s:property value="#gift_code.parent_code" />
                </td>
                 <td >
                        <s:property value="#gift_code.create_time" />
                </td>
                 <td >
                        <s:property value="#gift_code.add_user_id" />
                </td>
                    <td>
                             <s:url id="url_deletegift_code" action="gift_coderemove">
                             <s:param name="gift_code.pk_id"><s:property value="#gift_code.pk_id" /></s:param>
                             <s:param name="name_c"><s:property value="#request.name_c" /></s:param>
                               </s:url>
                               <s:a href="%{url_deletegift_code}" onclick="return del();">删除</s:a>
                        <font color="#808080" size="3">|</font>
                             <s:url id="url_updategift_code" action="gift_codeupdateP">
                             <s:param name="gift_code.pk_id"><s:property value="#gift_code.pk_id" /></s:param>
                             <s:param name="name_c"><s:property value="#request.name_c" /></s:param>
                               </s:url>
                               <s:a href="%{url_updategift_code}">修改</s:a>
                        <font color="#808080" size="3">|</font>
                        <s:a href="#">其他功能</s:a>
                    </td>
       ...
       <s:if test="#gift_code">
        ...
        <br>
         <center>
              <!-- 判断 gift_code对象是否为空的方法如下：-->
              <s:if test="!#gift_code">
                    没有查询到任何记录!
              </s:if>
        </center><br>
        <center>
                              <s:url id="url_excelgift_code" action="gift_codegenerateExcel">
                              <s:param name="name_c"><s:property value="#request.name_c" /></s:param>
                              </s:url>
                              <s:a href="%{url_excelgift_code}"><font color="#008000" size="3">生成 Excel 文件</font></s:a>
