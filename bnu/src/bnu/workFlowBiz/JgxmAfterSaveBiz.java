...
public class JgxmAfterSaveBiz extends WorkFlowStepRTClassA {
	
	public JgxmAfterSaveBiz(UserContext uc){
		super(uc);
		super.setDescription("科研情况_教改项目_保存成功:更改教工号,更新共享数据");
	...
	public boolean execute() {
		System.out.println(".....................科研情况_教改项目_保存:更改教工号,更新共享数据begin.....................");
		
		int workflowInstanceId = getParameter(this.PARAMETER_INSTANCE_ID).toInt();//获得当前工作流的id
		
		Hashtable rsTable = BOInstanceAPI.getInstance().getBOData("BO_JGXMTEMP", workflowInstanceId);
		...
		try{
			...
			String xmlx = rsTable.get("XMLX").toString() == null ? "": rsTable.get("XMLX").toString().trim();
			...
			String jtrq = rsTable.get("JTRQ").toString() == null ? "": rsTable.get("JTRQ").toString().trim();
			String zcr = rsTable.get("ZCR").toString() == null ? "": rsTable.get("ZCR").toString().trim();
			String pzjf = rsTable.get("PZJF").toString() == null ? "": rsTable.get("PZJF").toString().trim();
			String szdw = rsTable.get("SZDW").toString() == null ? "": rsTable.get("SZDW").toString().trim();
			String zcrid = rsTable.get("ZCRID").toString() == null ? "": rsTable.get("ZCRID").toString().trim();
			...
			int zcrrate = Integer.parseInt(rsTable.get("ZCRRATE").toString() == null ? "0" : rsTable.get("ZCRRATE").toString().trim());
...
//			}
			...
			loginId = rsTable.get("ZCRID").toString() == null ? "" : rsTable.get("ZCRID").toString().trim();
			...
			String updateInstanceSql = "update BO_JGXMTEMP set JGH ='" +jgh +"' "
			+ ", JGH1='" +jgh1 + "' "
			+ ", JGH2='" +jgh2 + "' "
			+ ", JGH3='" +jgh3 + "' "
			+ ", JGH4='" +jgh4 + "' "
...
//					System.out.println("deleteSql="+deleteSql);
			...
			if(loginId1 != null && !loginId1.equals("")) {
				...
				int createBoId = BOInstanceAPI.getInstance().createBOData("BO_JGXMTEMP", recordData, loginId1BindId, createuser);
				if(createBoId == -1) {
...
//				System.out.println("为 "+loginId1+" 共享数据：createBoId="+createBoId+", BINDID="+loginId1BindId);
				String updateCreateBoSql = "update BO_JGXMTEMP set "
    				+ "  XMMC ='" + xmmc +"' "
				    + ", XMLX ='" + xmlx +"' "
				    + ", XMLB ='" + xmlb +"' "
...
//				System.out.println("updateCreateBoSql="+updateCreateBoSql);
			...
			if(loginId2 != null && !loginId2.equals("")) {
				...
				int createBoId = BOInstanceAPI.getInstance().createBOData("BO_JGXMTEMP", recordData, loginId2BindId, createuser);
				if(createBoId == -1) {
					createBoFailureNum = createBoFailureNum + 1;
				} else {
					copyDataIds = copyDataIds + createBoId + ":";
...
//				System.out.println("updateCreateBoSql="+updateCreateBoSql);
			...
			if(loginId3 != null && !loginId3.equals("")) {
				...
				int createBoId = BOInstanceAPI.getInstance().createBOData("BO_JGXMTEMP", recordData, loginId3BindId, createuser);
				if(createBoId == -1) {
					createBoFailureNum = createBoFailureNum + 1;
				} else {
					copyDataIds = copyDataIds + createBoId + ":";
...
//				System.out.println("updateCreateBoSql="+updateCreateBoSql);
			...
			if(loginId4 != null && !loginId4.equals("")) {
				...
				int createBoId = BOInstanceAPI.getInstance().createBOData("BO_JGXMTEMP", recordData, loginId4BindId, createuser);
				if(createBoId == -1) {
					createBoFailureNum = createBoFailureNum + 1;
				} else {
					copyDataIds = copyDataIds + createBoId + ":";
...
//				System.out.println("updateCreateBoSql="+updateCreateBoSql);
			...
			if(loginId5 != null && !loginId5.equals("")) {
				...
				int createBoId = BOInstanceAPI.getInstance().createBOData("BO_JGXMTEMP", recordData, loginId5BindId, createuser);
				if(createBoId == -1) {
					createBoFailureNum = createBoFailureNum + 1;
				} else {
					copyDataIds = copyDataIds + createBoId + ":";
...
//				System.out.println("updateCreateBoSql="+updateCreateBoSql);
			...
			String updateCopyDataIdSql = "update BO_JGXMTEMP set copyDataId ='" +copyDataIds +"' where ID="+mainDataId;
