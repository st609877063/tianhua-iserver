...
public class ZdzyAprvAfterBiz extends WorkFlowStepRTClassA {
	
	public ZdzyAprvAfterBiz(UserContext uc){
		super(uc);
		super.setDescription("人才培养_指导专业_审核通过");
	...
	public boolean execute() {
		System.out.println("##########################人才培养_指导专业_审核通过#########################");
...
//		System.out.println("########人才培养_指导专业_审核#######workflowInstanceId=bindid="+workflowInstanceId+", #######id="+id);
...
//			String operUser = rsTable.get("CHARGEID").toString() == null ? "" : rsTable.get("CHARGEID").toString(); //获得用户
			String operUser = rsTable.get("DSBH").toString() == null ? "" : rsTable.get("DSBH").toString(); //获得用户
			...
			updateBindIdSql = " update BO_ZDZYTEMP set STATUS = 1, BINDID = "+bindId+" where ID=" + id;
			...
			if(r2<=0){
				...
				MessageQueue.getInstance().putMessage(getUserContext().getUID(), "警告，人才培养_指导专业 审批失败!", true);
				return false;
			}
			/****************************************************************************/
			...
			while(rs.next()) {
				...
				String weightSql = "select weight from BO_SELECTINFO " +
						" where TABLECD='BO_ZDZYTEMP' AND COLUMNCD='" +optionNmTemp + "' " +
			...
			updateStatusSql = " update BO_ZDZYTEMP set STATUS = 1, finalMark = "+finalMark+" where ID=" + id;
...
//			String name = rsTable.get("DSMC").toString() == null ? "" : rsTable.get("DSMC").toString();
//			String LoginId = rsTable.get("DSLOGINID").toString() == null ? "" : rsTable.get("DSLOGINID").toString();
//			String jgh = rsTable.get("DSBH").toString() == null ? "" : rsTable.get("DSBH").toString();
...
//			calMark.setFinalMark(name, LoginId, jgh, "BO_ZDZYTEMP", getMark);
//			String datayear = rsTable.get("DATAYEAR").toString()==null?"":rsTable.get("DATAYEAR").toString().trim();
//			calMark.setFinalMarkWithYear(name, LoginId, jgh, "BO_ZDZYTEMP", getMark, datayear);
			
			conn.commit();
		} catch (SQLException e) {
			e.printStackTrace();
			MessageQueue.getInstance().putMessage(getUserContext().getUID(), "人才培养_指导专业 权值设置失败！参考错误" + e.toString(), true);
			return false;
		} finally {
			try {
