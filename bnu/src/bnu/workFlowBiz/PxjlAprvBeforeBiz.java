...
public class PxjlAprvBeforeBiz extends WorkFlowStepRTClassA {
	
	public PxjlAprvBeforeBiz(UserContext uc){
		super(uc);
		super.setDescription("教育经历_培训经历");
...
//			String operUser = rsTable.get("CHARGEID").toString() == null ? "" : rsTable.get("CHARGEID").toString(); //获得用户
			...
			e.printStackTrace();
			MessageQueue.getInstance().putMessage(getUserContext().getUID(), "教育经历_培训经历 审批验证失败！参考错误" + e.toString(), true);
			return false;
		} finally {
			try {
				conn.setAutoCommit(true);
