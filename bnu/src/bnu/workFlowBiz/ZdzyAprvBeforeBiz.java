...
public class ZdzyAprvBeforeBiz extends WorkFlowStepRTClassA {
	
	public ZdzyAprvBeforeBiz(UserContext uc){
		super(uc);
		super.setDescription("人才培养_指导专业");
...
//			String operUser = rsTable.get("CHARGEID").toString() == null ? "" : rsTable.get("CHARGEID").toString(); //获得用户
			...
			e.printStackTrace();
			MessageQueue.getInstance().putMessage(getUserContext().getUID(), "人才培养_指导专业 审批验证失败！参考错误" + e.toString(), true);
			return false;
		} finally {
			try {
				conn.setAutoCommit(true);
