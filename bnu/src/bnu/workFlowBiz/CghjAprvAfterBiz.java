...
public class CghjAprvAfterBiz extends WorkFlowStepRTClassA {
	...
	public CghjAprvAfterBiz(UserContext uc){
		super(uc);
		super.setDescription("科研情况_成果获奖_审核通过");
	...
	public boolean execute() {
		System.out.println("##########################科研情况_成果获奖_审核通过#########################");
...
//		System.out.println("########科研情况_成果获奖_审核#######workflowInstanceId=bindid="+workflowInstanceId+", #######id="+id);
		...
		try {
			...
			if(r2<=0){
				...
				MessageQueue.getInstance().putMessage(getUserContext().getUID(), "警告，科研情况_成果获奖 审批失败!", true);
				return false;
			}
			/****************************************************************************/
...
//			calMark.setFinalMark(name, LoginId, jgh, "BO_CGHJTEMP", getMark);
			...
			e.printStackTrace();
			MessageQueue.getInstance().putMessage(getUserContext().getUID(), "科研情况_成果获奖 权值设置失败！参考错误" + e.toString(), true);
			return false;
		} finally {
			try {
