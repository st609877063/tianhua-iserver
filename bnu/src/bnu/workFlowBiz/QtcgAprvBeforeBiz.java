...
public class QtcgAprvBeforeBiz extends WorkFlowStepRTClassA {
	
	public QtcgAprvBeforeBiz(UserContext uc){
		super(uc);
		super.setDescription("科研情况_其他成果");
...
//		System.out.println("########科研情况_其他成果_审核#######workflowInstanceId=bindid="+workflowInstanceId+", #######id="+id);
		...
		try {
			...
			e.printStackTrace();
			MessageQueue.getInstance().putMessage(getUserContext().getUID(), "科研情况_其他成果 审批验证失败！参考错误" + e.toString(), true);
			return false;
		} finally {
			try {
				conn.setAutoCommit(true);
