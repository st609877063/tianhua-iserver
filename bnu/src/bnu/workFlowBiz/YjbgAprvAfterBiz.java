...
public class YjbgAprvAfterBiz extends WorkFlowStepRTClassA {
	
	public YjbgAprvAfterBiz(UserContext uc){
		super(uc);
		super.setDescription("科研情况_研究报告_审核通过");
	...
	public boolean execute() {
		System.out.println(".....................科研情况_研究报告_审核通过.....................");
...
//		System.out.println("########科研情况_研究报告_审核#######workflowInstanceId=bindid="+workflowInstanceId+", #######id="+id);
		...
		try {
			...
			updateBindIdSql = " update BO_YJBGTEMP set STATUS = 1, BINDID = "+bindId+" where ID=" + id;
			...
			if(r2<=0){
				...
				MessageQueue.getInstance().putMessage(getUserContext().getUID(), "警告，科研情况_研究报告 审批失败!", true);
				return false;
			}
			/****************************************************************************/
			...
			while(rs.next()) {
				...
				String weightSql = "select weight from BO_SELECTINFO " +
						" where TABLECD='BO_YJBGTEMP' AND COLUMNCD='" +optionNmTemp + "' " +
			...
			updateSql = " update BO_YJBGTEMP set STATUS = 1, BINDID = "+bindId+", finalMark = "+finalMark+" where ID=" + id;
...
//			calMark.setFinalMark(name, LoginId, jgh, "BO_YJBGTEMP", getMark);
			String datayear = rsTable.get("DATAYEAR").toString()==null?"":rsTable.get("DATAYEAR").toString().trim();
			calMark.setFinalMarkWithYear(name, LoginId, jgh, "BO_YJBGTEMP", getMark, datayear);
			
			conn.commit();
		} catch (SQLException e) {
			e.printStackTrace();
			MessageQueue.getInstance().putMessage(getUserContext().getUID(), "科研情况_研究报告 审批失败！参考错误" + e.toString(), true);
			return false;
		} finally {
			try {
