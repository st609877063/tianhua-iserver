...
public class InitDataAfterExcel extends WorkFlowStepRTClassA {
	...
	public InitDataAfterExcel(UserContext uc){
		super(uc);
		super.setDescription("初始化基本信息。导入Excel到副表BO_ECNU_PEOPLE2后，为每条数据新建流程写入正式表BO_ECNU_PEOPLE中");
		super.setVersion("V1.0");
		super.setProvider("maxwell");
	}

	public boolean execute() {
		System.out.println(".....................初始化基本信息(导入副表BO_ECNU_PEOPLE2，再入主表)begin..............");
		...
		try{
			...
			String sql = "select * from BO_ECNU_PEOPLE2";
			...
			while(rs.next()){
				...
				int checkId = 0;
				String checkSql = "select ID FROM BO_ECNU_PEOPLE WHERE LOGINID = '"+LOGINID + "' and ZGH='" +ZGH+ "'";
				...
				if(checkId == 0) { //新增
					...
					int boId = BOInstanceAPI.getInstance().createBOData("BO_ECNU_PEOPLE", recordData, "admin");
					int boInstanceId = WorkflowInstanceAPI.getInstance().createProcessInstance("ff2a4340bdd5692652f152a79e79c39c", "admin", "华东师大基本信息填报");
					//ff2a4340bdd5692652f152a79e79c39c   ===>  基本信息填报流程 (开发)
					// ff2a4340bdd5692652f152a79e79c39c  ===>  基本信息填报流程  （online）
//					System.out.println("新增数据：BINDID = "+boInstanceId+", ID="+boId);
					String updateInstanceSql = "update BO_ECNU_PEOPLE set ZGH ='" +ZGH +"' "
							+ ", LOGINID='" + LOGINID +"' "
							+ ", XM='" + XM +"' "
							+ ", XB='" + XB +"' "
...
//					System.out.println("updateInstanceSql="+updateInstanceSql);
					...
					String updateSql = "update BO_ECNU_PEOPLE set ZGH ='" +ZGH +"' "
										+ ", LOGINID='" + LOGINID +"' "
										+ ", XM='" + XM +"' "
										+ ", XB='" + XB +"' "
										+ ", CSNY = TO_DATE('" +  CSNY +"','YYYY-MM-DD') " 
