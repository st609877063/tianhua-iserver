...
import com.actionsoft.awf.organization.dao.UserMap;
...
*/
public class HRZLZZAddressWeb extends ActionsoftWeb {
	/**
	 * 构造方法
	 * @param userContext 用户上下文对象
	 */
	public HRZLZZAddressWeb(UserContext userContext) {
		super(userContext);
	...
	public String getAddressTree(String awsuid,String sapuid,int companyId, int departmentId) {
		...
		if(userMap.size() > 0) {
			for(int i = 0; i < userMap.size(); i++) {
				...
				System.out.println(roleModel._roleName.indexOf("部门助理"));
				if(roleModel._roleName.indexOf("部门助理") > -1) {
					//当前用户兼职部门
		...
		return RepleaseKey.replace(HtmlModelFactory.getHtmlModel("Kingsoft_HR_ZLZZ_Tree.htm"), hashTags);
	}
