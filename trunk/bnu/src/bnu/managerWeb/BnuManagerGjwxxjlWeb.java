...
public class BnuManagerGjwxxjlWeb extends ActionsoftWeb {
	/**
	 * 构造方法
	 * @param userContext 用户上下文对象
	 */
	public BnuManagerGjwxxjlWeb(UserContext userContext) {
		super(userContext);
	...
	public String getReport(int pageNow,int lineNumber,String awsuid) {
		...
		String sqlw = "select * from BO_GJWXXJLTEMP order by CREATEDATE desc";
		String sqlc = "select count(*) c from BO_GJWXXJLTEMP ";
...
//			   		"order by CREATEDATE desc";
		   ...
		   if (lineCount > lineNumber) {
			   ...
			   .append(new PageIndex("BNU_MANAGER_GJWXXJL", pageNow, lineCount, lineNumber).toString())
			   .append("</div></td></tr>");
		   }
		...
		return RepleaseKey.replace(HtmlModelFactory.getHtmlModel("BNU_MANAGER_GJWXXJL.html"),hashTags);
	}
