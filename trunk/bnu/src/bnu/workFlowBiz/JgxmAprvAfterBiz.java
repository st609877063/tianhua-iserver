...
public class JgxmAprvAfterBiz extends WorkFlowStepRTClassA {
	
	public JgxmAprvAfterBiz(UserContext uc){
		super(uc);
		super.setDescription("人才培养_教改项目_审核通过");
	...
	public boolean execute() {
		System.out.println("##########################人才培养_教改项目_审核通过#########################");
...
//		System.out.println("########人才培养_教改项目_审核#######workflowInstanceId=bindid="+workflowInstanceId+", #######id="+id);
...
//			String operUser = rsTable.get("CHARGEID").toString() == null ? "" : rsTable.get("CHARGEID").toString(); //获得用户
			String operUser = rsTable.get("ZCRID").toString() == null ? "" : rsTable.get("ZCRID").toString(); //获得用户
			...
			updateBindIdSql = " update BO_JGXMTEMP set STATUS = 1, BINDID = "+bindId+" where ID=" + id;
			...
			if(r2<=0){
				...
				MessageQueue.getInstance().putMessage(getUserContext().getUID(), "警告，人才培养_教改项目 审批失败!", true);
				return false;
			}
			/****************************************************************************/
			...
			while(rs.next()) {
				...
				String weightSql = "select weight from BO_SELECTINFO " +
						" where TABLECD='BO_JGXMTEMP' AND COLUMNCD='" +optionNmTemp + "' " +
			...
			updateStatusSql = " update BO_JGXMTEMP set STATUS = 1, finalMark = "+finalMark+" where ID=" + id;
			...
			String name = rsTable.get("ZCR").toString() == null ? "" : rsTable.get("ZCR").toString();
			String LoginId = rsTable.get("ZCRID").toString() == null ? "" : rsTable.get("ZCRID").toString();
...
//			calMark.setFinalMark(name, LoginId, jgh, "BO_JGXMTEMP", getMark);
			String datayear = rsTable.get("DATAYEAR").toString()==null?"":rsTable.get("DATAYEAR").toString().trim();
			calMark.setFinalMarkWithYear(name, LoginId, jgh, "BO_JGXMTEMP", getMark, datayear);
			
			conn.commit();
		} catch (SQLException e) {
			e.printStackTrace();
			MessageQueue.getInstance().putMessage(getUserContext().getUID(), "人才培养_教改项目 权值设置失败！参考错误" + e.toString(), true);
			return false;
		} finally {
			try {
