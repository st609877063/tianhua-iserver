...
public class CghjAprvBeforeBiz extends WorkFlowStepRTClassA {
	
	public CghjAprvBeforeBiz(UserContext uc){
		super(uc);
		super.setDescription("科研情况_成果获奖");
...
//		System.out.println("########科研情况_成果获奖_审核#######workflowInstanceId=bindid="+workflowInstanceId+", #######id="+id);
		...
		try {
			...
			e.printStackTrace();
			MessageQueue.getInstance().putMessage(getUserContext().getUID(), "科研情况_成果获奖 审批验证失败！参考错误" + e.toString(), true);
			return false;
		} finally {
			try {
				conn.setAutoCommit(true);
