...
public class GzjlAprvBeforeBiz extends WorkFlowStepRTClassA {
	
	public GzjlAprvBeforeBiz(UserContext uc){
		super(uc);
		super.setDescription("工作经历");
...
//		System.out.println("########工作经历_审核#######workflowInstanceId=bindid="+workflowInstanceId+", #######id="+id);
		...
		try {
			...
			e.printStackTrace();
			MessageQueue.getInstance().putMessage(getUserContext().getUID(), "工作经历 审批验证失败！参考错误" + e.toString(), true);
			return false;
		} finally {
			try {
				conn.setAutoCommit(true);
