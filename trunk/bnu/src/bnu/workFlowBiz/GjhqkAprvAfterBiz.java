...
public class GjhqkAprvAfterBiz extends WorkFlowStepRTClassA {
	
	public GjhqkAprvAfterBiz(UserContext uc){
		super(uc);
		super.setDescription("国际化情况_审核通过");
...
//		System.out.println("########国际化情况_审核#######workflowInstanceId=bindid="+workflowInstanceId+", #######id="+id);
...
//			String operUser = rsTable.get("CHARGEID").toString() == null ? "" : rsTable.get("CHARGEID").toString(); //获得用户
			...
			updateBindIdSql = " update BO_GJHQKTEMP set STATUS = 1, BINDID = "+bindId+" where ID=" + id;
			...
			if(r2<=0){
				...
				MessageQueue.getInstance().putMessage(getUserContext().getUID(), "警告，国际化情况 审批失败!", true);
				return false;
			}
			/****************************************************************************/
			...
			/*String sql = "select b.field_name as optionNm from SYS_BUSINESS_METADATA  a, SYS_BUSINESS_METADATA_MAP b " +
					...
					" and a.entity_name='BO_GJHQKTEMP'";
			...
			while(rs.next()) {
				...
				String weightSql = "select weight from BO_SELECTINFO " +
						" where TABLECD='BO_GJHQKTEMP' AND COLUMNCD='" +optionNmTemp + "' " +
			...
			updateStatusSql = " update BO_GJHQKTEMP set STATUS = 1, finalMark = "+finalMark+" where ID=" + id;
...
//			calMark.setFinalMark(name, LoginId, jgh, "BO_GJHQKTEMP", getMark);	
			String datayear = rsTable.get("DATAYEAR").toString()==null?"":rsTable.get("DATAYEAR").toString().trim();
			calMark.setFinalMarkWithYear(name, LoginId, jgh, "BO_GJHQKTEMP", getMark, datayear);
			
			conn.commit();
		} catch (SQLException e) {
			e.printStackTrace();
			MessageQueue.getInstance().putMessage(getUserContext().getUID(), "国际化情况 权值设置失败！参考错误" + e.toString(), true);
			return false;
		} finally {
			try {
