...
public class RkqkAprvAfterBiz extends WorkFlowStepRTClassA {
	
	public RkqkAprvAfterBiz(UserContext uc){
		super(uc);
		super.setDescription("人才培养_任课情况_审核通过");
	...
	public boolean execute() {
		System.out.println("##########################人才培养_任课情况_审核通过#########################");
...
//		System.out.println("########人才培养_任课情况_审核#######workflowInstanceId=bindid="+workflowInstanceId+", #######id="+id);
		...
		try {
			...
			updateBindIdSql = " update BO_RKQKTEMP set STATUS = 1, BINDID = "+bindId+" where ID=" + id;
			...
			if(r2<=0){
				...
				MessageQueue.getInstance().putMessage(getUserContext().getUID(), "警告，人才培养_任课情况 审批失败!", true);
				return false;
			}
			/****************************************************************************/
			...
			while(rs.next()) {
				...
				String weightSql = "select weight from BO_SELECTINFO " +
						" where TABLECD='BO_RKQKTEMP' AND COLUMNCD='" +optionNmTemp + "' " +
								" AND OPTIONNM='" +optionValue+ "'";
				
				int weight = DBSql.getInt(weightSql, "weight");
				
				String timeStr = rs.getString("ZXS")==null ? "" : rs.getString("ZXS");
			...
			updateStatusSql = " update BO_RKQKTEMP set STATUS = 1, finalMark = "+finalMark+" where ID=" + id;
...
//			calMark.setFinalMark(name, LoginId, jgh, "BO_RKQKTEMP", getMark);
			String datayear = rsTable.get("DATAYEAR").toString()==null?"":rsTable.get("DATAYEAR").toString().trim();
			calMark.setFinalMarkWithYear(name, LoginId, jgh, "BO_RKQKTEMP", getMark, datayear);
			
			conn.commit();
		} catch (SQLException e) {
			e.printStackTrace();
			MessageQueue.getInstance().putMessage(getUserContext().getUID(), "人才培养_任课情况 权值设置失败！参考错误" + e.toString(), true);
			return false;
		} finally {
			try {
