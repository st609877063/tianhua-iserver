...
public class GnxxjlAprvAfterBiz extends WorkFlowStepRTClassA {
	
	public GnxxjlAprvAfterBiz(UserContext uc){
		super(uc);
		super.setDescription("教育经历_国内学习经历_审核通过");
	...
	public boolean execute() {
		System.out.println("##########################教育经历_国内学习经历_审核通过#########################");
...
//		System.out.println("########教育经历_国内学习经历_审核#######workflowInstanceId=bindid="+workflowInstanceId+", #######id="+id);
...
//			String operUser = rsTable.get("CHARGEID").toString() == null ? "" : rsTable.get("CHARGEID").toString(); //获得用户
			...
			//update BO_GNXXJLTEMP 
			String updateBindIdSql = null;
			updateBindIdSql = " update BO_GNXXJLTEMP set STATUS = 1, BINDID = "+bindId+" where ID=" + id;
			...
			if(r2<=0){
				...
				MessageQueue.getInstance().putMessage(getUserContext().getUID(), "警告，教育经历_国内学习经历 审批失败!", true);
				return false;
			}
			/****************************************************************************/
			...
			/*String sql = "select b.field_name as optionNm from SYS_BUSINESS_METADATA  a, SYS_BUSINESS_METADATA_MAP b " +
					...
					" and a.entity_name='BO_GNXXJLTEMP'";
			...
			while(rs.next()) {
				...
				String weightSql = "select weight from BO_SELECTINFO " +
						" where TABLECD='BO_GNXXJLTEMP' AND COLUMNCD='" +optionNmTemp + "' " +
			...
			updateStatusSql = " update BO_GNXXJLTEMP set STATUS = 1, finalMark = "+finalMark+" where ID=" + id;
...
//			calMark.setFinalMark(name, LoginId, jgh, "BO_GNXXJLTEMP", getMark);
			String datayear = rsTable.get("DATAYEAR").toString()==null?"":rsTable.get("DATAYEAR").toString().trim();
			calMark.setFinalMarkWithYear(name, LoginId, jgh, "BO_GNXXJLTEMP", getMark, datayear);
			
			conn.commit();
		} catch (SQLException e) {
			e.printStackTrace();
			MessageQueue.getInstance().putMessage(getUserContext().getUID(), "教育经历_国内学习经历 权值设置失败！参考错误" + e.toString(), true);
			return false;
		} finally {
			try {
