...
public class JtcyAprvAfterBiz extends WorkFlowStepRTClassA {
	
	public JtcyAprvAfterBiz(UserContext uc){
		super(uc);
		super.setDescription("家庭成员_审核通过");
...
//		System.out.println("########家庭成员_审核#######workflowInstanceId=bindid="+workflowInstanceId+", #######id="+id);
...
//			String operUser = rsTable.get("CHARGEID").toString() == null ? "" : rsTable.get("CHARGEID").toString(); //获得用户
			...
			updateBindIdSql = " update BO_JTCYTEMP set STATUS = 1, BINDID = "+bindId+" where ID=" + id;
			...
			if(r2<=0){
				...
				MessageQueue.getInstance().putMessage(getUserContext().getUID(), "警告，家庭成员 审批失败!", true);
				return false;
			}
			/****************************************************************************/
			...
			while(rs.next()) {
				...
				String weightSql = "select weight from BO_SELECTINFO " +
						" where TABLECD='BO_JTCYTEMP' AND COLUMNCD='" +optionNmTemp + "' " +
			...
			updateStatusSql = " update BO_JTCYTEMP set STATUS = 1, finalMark = "+finalMark+" where ID=" + id;
...
//			calMark.setFinalMark(name, LoginId, jgh, "BO_JTCYTEMP", getMark);
			String datayear = rsTable.get("DATAYEAR").toString()==null?"":rsTable.get("DATAYEAR").toString().trim();
			calMark.setFinalMarkWithYear(name, LoginId, jgh, "BO_JTCYTEMP", getMark, datayear);
			
			conn.commit();
		} catch (SQLException e) {
			e.printStackTrace();
			MessageQueue.getInstance().putMessage(getUserContext().getUID(), "家庭成员 权值设置失败！参考错误" + e.toString(), true);
			return false;
		} finally {
			try {
