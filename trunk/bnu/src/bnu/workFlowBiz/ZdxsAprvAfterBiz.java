...
public class ZdxsAprvAfterBiz extends WorkFlowStepRTClassA {
	
	public ZdxsAprvAfterBiz(UserContext uc){
		super(uc);
		super.setDescription("人才培养_指导学生_审核通过");
	...
	public boolean execute() {
		System.out.println("##########################人才培养_指导学生_审核通过#########################");
...
//		System.out.println("########人才培养_指导学生_审核#######workflowInstanceId=bindid="+workflowInstanceId+", #######id="+id);
		...
		try {
			...
			updateBindIdSql = " update BO_ZDXSTEMP set STATUS = 1, BINDID = "+bindId+" where ID=" + id;
			...
			if(r2<=0){
				...
				MessageQueue.getInstance().putMessage(getUserContext().getUID(), "警告，人才培养_指导学生 审批失败!", true);
				return false;
			}
			/****************************************************************************/
			...
			while(rs.next()) {
				...
				String weightSql = "select weight from BO_SELECTINFO " +
						" where TABLECD='BO_ZDXSTEMP' AND COLUMNCD='" +optionNmTemp + "' " +
								" AND OPTIONNM='" +optionValue+ "'";
				
				int weight = DBSql.getInt(weightSql, "weight");
				
				String timeStr = rs.getString("TIME")==null ? "" : rs.getString("TIME");
			...
			updateStatusSql = " update BO_ZDXSTEMP set STATUS = 1, finalMark = "+finalMark+" where ID=" + id;
			...
			if(markStr!=null && !markStr.equals("")) {
				getMark = Float.parseFloat(markStr);
			}
			CalMark calMark = new CalMark();
//			calMark.setFinalMark(name, LoginId, jgh, "BO_ZDXSTEMP", getMark);
			String datayear = rsTable.get("DATAYEAR").toString()==null?"":rsTable.get("DATAYEAR").toString().trim();
			calMark.setFinalMarkWithYear(name, LoginId, jgh, "BO_ZDXSTEMP", getMark, datayear);
			
			conn.commit();
		} catch (SQLException e) {
			e.printStackTrace();
			MessageQueue.getInstance().putMessage(getUserContext().getUID(), "人才培养_指导学生 权值设置失败！参考错误" + e.toString(), true);
			return false;
		} finally {
			try {
