...
public class XslwAprvBeforeBiz extends WorkFlowStepRTClassA {
	
	public XslwAprvBeforeBiz(UserContext uc){
		super(uc);
		super.setDescription("科研情况_学术论文");
...
//		System.out.println("########科研情况_学术论文_审核#######workflowInstanceId=bindid="+workflowInstanceId+", #######id="+id);
		...
		try {
			...
			/***********************查找是否有个人基本信息****************************/
			String chargeSf = rsTable.get("CHARGESF").toString() == null ? "" : rsTable.get("CHARGESF").toString(); //获得身份
			System.out.println("chargeSf===="+chargeSf);
			//本校教师，本校学生，外单位，其他
			...
			e.printStackTrace();
			MessageQueue.getInstance().putMessage(getUserContext().getUID(), "科研情况_学术论文 审批验证失败！参考错误" + e.toString(), true);
			return false;
		} finally {
			try {
				conn.setAutoCommit(true);
