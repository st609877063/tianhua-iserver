...
public class GjwxxjlAprvAfterBiz extends WorkFlowStepRTClassA {
	
	public GjwxxjlAprvAfterBiz(UserContext uc){
		super(uc);
		super.setDescription("教育经历_国境外学习经历_审核通过");
	...
	public boolean execute() {
		System.out.println("##########################教育经历_国境外学习经历_审核通过#########################");
...
//		System.out.println("########教育经历_国境外学习经历_审核#######workflowInstanceId=bindid="+workflowInstanceId+", #######id="+id);
...
//			String operUser = rsTable.get("CHARGEID").toString() == null ? "" : rsTable.get("CHARGEID").toString(); //获得用户
			String operUser = rsTable.get("CREATEUSER").toString() == null ? "" : rsTable.get("CREATEUSER").toString(); //获得用户
			...
			//update BO_GJWXXJLTEMP 
			String updateBindIdSql = null;
			updateBindIdSql = " update BO_GJWXXJLTEMP set STATUS = 1, BINDID = "+bindId+" where ID=" + id;
			...
			if(r2<=0){
				...
				MessageQueue.getInstance().putMessage(getUserContext().getUID(), "警告，教育经历_国境外学习经历 审批失败!", true);
				return false;
			}
			/****************************************************************************/
			...
			/*String sql = "select b.field_name as optionNm from SYS_BUSINESS_METADATA  a, SYS_BUSINESS_METADATA_MAP b " +
					...
					" and a.entity_name='BO_GJWXXJLTEMP'";
			...
			while(rs.next()) {
				...
				String weightSql = "select weight from BO_SELECTINFO " +
						" where TABLECD='BO_GJWXXJLTEMP' AND COLUMNCD='" +optionNmTemp + "' " +
			...
			updateStatusSql = " update BO_GJWXXJLTEMP set STATUS = 1, finalMark = "+finalMark+" where ID=" + id;
			...
			String name = "";
			String LoginId = rsTable.get("CREATEUSER").toString() == null ? "" : rsTable.get("CREATEUSER").toString();
			String jgh = "";
...
//			calMark.setFinalMark(name, LoginId, jgh, "BO_GJWXXJLTEMP", getMark);
			String datayear = rsTable.get("DATAYEAR").toString()==null?"":rsTable.get("DATAYEAR").toString().trim();
			calMark.setFinalMarkWithYear(name, LoginId, jgh, "BO_GJWXXJLTEMP", getMark, datayear);
			
			conn.commit();
		} catch (SQLException e) {
			e.printStackTrace();
			MessageQueue.getInstance().putMessage(getUserContext().getUID(), "教育经历_国境外学习经历 权值设置失败！参考错误" + e.toString(), true);
			return false;
		} finally {
			try {
