...
public class GnxxjlAprvBeforeBiz extends WorkFlowStepRTClassA {
	
	public GnxxjlAprvBeforeBiz(UserContext uc){
		super(uc);
		super.setDescription("教育经历_国内学习经历");
	...
	public boolean execute() {
		System.out.println("##########################教育经历_国内学习经历#########################");
...
//			String operUser = rsTable.get("CHARGEID").toString() == null ? "" : rsTable.get("CHARGEID").toString(); //获得用户
			...
			e.printStackTrace();
			MessageQueue.getInstance().putMessage(getUserContext().getUID(), "教育经历_国内学习经历 审批验证失败！参考错误" + e.toString(), true);
			return false;
		} finally {
			try {
				conn.setAutoCommit(true);
