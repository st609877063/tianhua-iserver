...
<html>
    <head>
            <title>Gift_group_operation 操作主界面</title>
<style>
.tdLabel {
    text-align: right;
}
    ...
    <%
      //和ListGift_group_operationByGroup_idAction.java文件里定义的默认值保持一致即可
      // 定义如下分页变量
    ...
    <%
     ...
     %>
        ...
        <br>
        <s:form name="form1" action="gift_group_operationlistByGroup_id.action" method="post" >
            <table border="1" style="BORDER-COLLAPSE: collapse" width="35%"
                align="center" bgcolor="#FFFFCC" >
                <tr align=center  bgcolor="#CCFF66">
                    <td colspan="2">
                        Gift_group_operation 自定义查询条件
                    </td>
                     <s:textfield id="group_id_c" name="group_id_c" label="group_id"></s:textfield>
        ...
        <table width="80%" align="center">
         <s:url id="url_savegift_group_operation" action="gift_group_operationsaveP">
             <s:param name="group_id_c"><s:property value="#request.group_id_c" /></s:param>
         </s:url>
         <s:a href="%{url_savegift_group_operation}">添加新记录</s:a>
        </table>
        <hr color="#CCCCFF">
        <table border="1" width="80%" align="center" bgcolor="#FFFFCC"
            ...
            <s:iterator value="#request.list" id="gift_group_operation" >
                <tr align="center" onmousemove="return this.bgColor='#FDF525';"  onmouseout="return this.bgColor='';">
                 <td >
                        <s:property value="#gift_group_operation.go_id" />
                </td>
                 <td >
                        <s:property value="#gift_group_operation.group_id" />
                </td>
                 <td >
                        <s:property value="#gift_group_operation.oper_id" />
                </td>
                 <td >
                        <s:property value="#gift_group_operation.add_time" />
                </td>
                 <td >
                        <s:property value="#gift_group_operation.add_user_id" />
                </td>
                    <td>
                             <s:url id="url_deletegift_group_operation" action="gift_group_operationremove">
                             <s:param name="gift_group_operation.go_id"><s:property value="#gift_group_operation.go_id" /></s:param>
                             <s:param name="group_id_c"><s:property value="#request.group_id_c" /></s:param>
                               </s:url>
                               <s:a href="%{url_deletegift_group_operation}" onclick="return del();">删除</s:a>
                        <font color="#808080" size="3">|</font>
                             <s:url id="url_updategift_group_operation" action="gift_group_operationupdateP">
                             <s:param name="gift_group_operation.go_id"><s:property value="#gift_group_operation.go_id" /></s:param>
                             <s:param name="group_id_c"><s:property value="#request.group_id_c" /></s:param>
                               </s:url>
                               <s:a href="%{url_updategift_group_operation}">修改</s:a>
                        <font color="#808080" size="3">|</font>
                        <s:a href="#">其他功能</s:a>
                    </td>
       ...
       <s:if test="#gift_group_operation">
          <% if (pageSize!=0) {%>
          <tr align="right">
              <td colspan=6>
        ...
        <br>
         <center>
              <!-- 判断 gift_group_operation对象是否为空的方法如下：-->
              <s:if test="!#gift_group_operation">
                    没有查询到任何记录!
              </s:if>
        </center><br>
        <center>
                              <s:url id="url_excelgift_group_operation" action="gift_group_operationgenerateExcel">
                              <s:param name="group_id_c"><s:property value="#request.group_id_c" /></s:param>
                              </s:url>
                              <s:a href="%{url_excelgift_group_operation}"><font color="#008000" size="3">生成 Excel 文件</font></s:a>
